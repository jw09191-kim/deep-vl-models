...
class VJEPAVideoProcessor(Qwen3VLVideoProcessor):
    ...
    def fetch_videos(self, video_url_or_urls, sample_indices_fn=None, **kwargs):
        # fetch_videos in BaseVideoProcessor hardcodes backend="torchcodec" as a local
        # variable, so setting self.video_backend has no effect.  Override here to force
        # pyav, which decodes until stream exhaustion and tolerates videos whose container
        # metadata over-reports the frame count (torchcodec crashes in that case).
